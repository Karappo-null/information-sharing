...
export default function Home() {
  ...
  return <main className="min-h-screen bg-[#f6f8f7] text-[#16251f]"><header className="border-b border-[#dce5df] bg-white px-5 py-4 sm:px-8"><div className="mx-auto flex max-w-6xl items-center justify-between"><span className="text-xl font-bold">Shareboard<span className="text-[#1f7a54]">.</span></span><div className="flex items-center gap-3 text-sm"><span className="hidden text-[#537064] sm:block">{currentUser.display_name}{isAdmin && "（管理者）"}</span><button className="rounded-full border border-[#cbd9d0] px-4 py-2" onClick={() => void supabase.auth.signOut()}>ログアウト</button></div></div></header>
    <div className="mx-auto grid max-w-6xl gap-6 px-5 py-8 sm:px-8 lg:grid-cols-[300px_1fr]"><aside className="rounded-2xl border border-[#dce5df] bg-white p-4 shadow-sm lg:h-fit"><div className="mb-4 flex justify-between"><h1 className="font-semibold">トピック</h1><button className="text-sm font-semibold text-[#1f7a54]" onClick={() => setTopicForm("create")}>＋ 作成</button></div>{topics.length === 0 ? <p className="py-6 text-sm leading-6 text-[#71847b]">まだトピックがありません。最初の話題を作ってみましょう。</p> : <nav className="space-y-1">{topics.map((topic) => <button key={topic.id} onClick={() => void selectTopic(topic)} className={`w-full rounded-xl px-3 py-3 text-left text-sm ${activeTopic?.id === topic.id ? "bg-[#e7f3eb] font-semibold text-[#176441]" : "hover:bg-[#f4f7f5]"}`}><span className="block truncate">{topic.title}</span>{topic.description && <span className="mt-1 block truncate text-xs font-normal text-[#71847b]">{topic.description}</span>}</button>)}</nav>}</aside>
      <section>{activeTopic ? <><div className="mb-5"><p className="mb-2 text-sm font-semibold text-[#1f7a54]">DISCUSSION</p><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-3xl font-bold sm:text-4xl">{activeTopic.title}</h2><p className="mt-2 text-sm text-[#71847b]">作成者: {activeTopic.profiles?.[0]?.display_name ?? "メンバー"}</p></div><div className="flex gap-2"><button className="rounded-full border border-[#cbd9d0] px-3 py-2 text-sm" onClick={() => setTopicForm("edit")}>編集</button>{isAdmin && <button className="rounded-full border border-red-200 px-3 py-2 text-sm text-red-700" onClick={() => void deleteTopic()} disabled={busy}>削除</button>}</div></div>{activeTopic.description && <p className="mt-3 text-[#537064]">{activeTopic.description}</p>}</div><div className="mb-4 text-sm text-[#71847b]">{posts.length} 件の投稿</div><div className="space-y-3">{posts.length === 0 ? <EmptyPosts /> : posts.map((post) => <PostCard key={post.id} post={post} user={currentUser} isAdmin={isAdmin} imageUrls={imageUrls} onEdit={setEditingPost} onDelete={deletePost} onDeleteImage={deleteImage} onEditComment={setEditingComment} onDeleteComment={deleteComment} onSaveComment={saveComment} busy={busy} />)}</div><form onSubmit={createPost} className="mt-5 rounded-2xl border border-[#dce5df] bg-white p-4 shadow-sm"><label className="mb-2 block text-sm font-semibold">投稿を書く</label><textarea name="body" required maxLength={5000} rows={4} placeholder="共有したいこと、質問、補足などを書いてください" className="w-full rounded-xl border border-[#cbd9d0] p-3 text-sm" /><label className="mt-3 block text-sm text-[#537064]">画像（任意・最大4枚・JPEG／PNG／WebP・各5MBまで）<input name="images" type="file" accept="image/jpeg,image/png,image/webp" multiple className="mt-2 block w-full text-sm" /></label><div className="mt-3 flex justify-end"><button disabled={busy} className="rounded-full bg-[#1f7a54] px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-50">{busy ? "保存中…" : "投稿する"}</button></div></form></> : <div className="rounded-2xl border border-dashed border-[#cbd9d0] bg-white p-12 text-center"><h2 className="text-xl font-bold">共有を始めましょう</h2><p className="mt-2 text-sm text-[#71847b]">左側からトピックを作成すると、投稿を追加できます。</p></div>}</section></div>
...
function Setup() { return <main className="grid min-h-screen place-items-center bg-[#f6f8f7] p-6 text-[#16251f]"><div className="max-w-lg rounded-2xl border border-[#dce5df] bg-white p-8 shadow-sm"><p className="text-sm font-semibold text-[#1f7a54]">SHAREBOARD</p><h1 className="mt-2 text-3xl font-bold">Supabase を接続してください</h1><p className="mt-4 leading-7 text-[#537064]">環境変数を設定すると、ログイン、Topic、Post、画像添付を利用できます。</p><code className="mt-6 block rounded-xl bg-[#eef3f0] p-4 text-sm">NEXT_PUBLIC_SUPABASE_URL<br />NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY</code></div></main>; }
